...
{
	var textExtensions : string[] = (vscode.workspace.getConfiguration().get("UWAP.pluginFilePacker.textExtensions") as string).split(/[\s,;]+/);
	...
	var namespace : string = await getNamespaceFromFile(vscode.Uri.joinPath(projUri, "FileHandler.cs")) ?? await getNamespaceFromFile(vscode.Uri.joinPath(projUri, "FileHandlerCustom.cs")) ?? "uwap.WebFramework.Plugins";
	var className : string = await getClassFromFile(vscode.Uri.joinPath(projUri, "FileHandler.cs")) ?? await getClassFromFile(vscode.Uri.joinPath(projUri, "FileHandlerCustom.cs")) ?? projName;
	var files = await fs.getFilesRecursive(vscode.Uri.joinPath(projUri, "Files"));
	var pathIndex : number = projUri.path.length + 6;
	...
	var customExists : boolean = await fs.exists(vscode.Uri.joinPath(projUri, "FileHandlerCustom.cs"));
	for (var file of files)
	{
		var relPath : string = file.path.substring(pathIndex);
		var bytes : Uint8Array = await fs.readBytes(file);
		if (textExtensions.some(x => relPath.endsWith(x)))
		{
			var text : string = new TextDecoder().decode(bytes);
...
async function getNamespaceFromFile(path : vscode.Uri) : Promise<string | undefined>
{
...
async function getClassFromFile(path : vscode.Uri) : Promise<string | undefined>
{
	if (!(await fs.exists(path)))
	{
		return undefined;
	...
	for (var line of await fs.readLines(path))
	{
		var index : number = line.indexOf("class ");
